...
export async function POST(request: Request) {
  ...
  try {
    ...
    switch (body.action) {
      ...
      case 'sync': data = await synchronize(s, body.settings, body.scope); break
      case 'assistant': case 'propose':
        ...
        data = body.action === 'assistant' ? await assistant(s, body.prompt) : await propose(s, body.path, body.prompt); break
      case 'approve': case 'reject':
        if (body.action === 'approve' && body.confirmed !== true) return Response.json({ error: 'Se requiere aprobación explícita.' }, { status: 403 })
        data = await decide(s, body.id, body.action === 'approve'); break
